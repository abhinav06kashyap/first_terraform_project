...
resource "azurerm_lb" "ak_lb" {
 name                = "lb-${local.sufix_for_resourse}"
  resource_group_name = azurerm_resource_group.abhinav_rg.name
  location            = azurerm_resource_group.abhinav_rg.location

  frontend_ip_configuration {
    name                 = "PublicIPAddress"
    public_ip_address_id = azurerm_public_ip.ak-lb-public-ip.id
  }
}

#create an backend pool
resource "azurerm_lb_backend_address_pool" "ak-lb_backend_pool" {
  loadbalancer_id = azurerm_lb.ak_lb.id #backendpooll need to attach with lb
  name            = "BackEndAddressPool"
} 

##probes health check up 
resource "azurerm_lb_probe" "ak-lb_probe" {
  loadbalancer_id = azurerm_lb.ak_lb.id
  name = "frontendprobe"
  protocol = "Tcp"
  #protocol = "Http"
...
resource "azurerm_lb_rule" "ak-lb_rule" {
  loadbalancer_id = azurerm_lb.ak_lb.id 
  name = "lbrule"
  protocol = "Tcp"
  frontend_port = 80 #lb 443
  backend_port = 80 #application port tomcat
  frontend_ip_configuration_name = azurerm_lb.ak_lb.frontend_ip_configuration[0].name 
  backend_address_pool_ids = [azurerm_lb_backend_address_pool.ak-lb_backend_pool.id]
  probe_id = azurerm_lb_probe.ak-lb_probe.id 
}
...
resource "azurerm_network_interface_backend_address_pool_association" "nic_backend_pool_association" {
 ...
 network_interface_id = azurerm_network_interface.abhinav_web_nic[each.key].id 
  ip_configuration_name = azurerm_network_interface.abhinav_web_nic[each.key].ip_configuration[0].name 
  backend_address_pool_id = azurerm_lb_backend_address_pool.ak-lb_backend_pool.id 
}